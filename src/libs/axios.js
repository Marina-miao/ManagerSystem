...
class HttpRequest {
  ...
  interceptors (instance, url) {
    ...
    instance.interceptors.request.use(config => {
      ...
      if (!/^\/|(http(s)?)/i.test(config.url)) config.url = `${/login/i.test(config.url) ? window.APP_CONFIG.loginPath : window.APP_CONFIG.appPath}/${config.url}`
    ...
    instance.interceptors.response.use(({ data: { sysCode, msg, data } }) => {
      ...
      switch (sysCode) {
        ...
        case 1002:
          ...
          return Promise.reject(new Error({ sysCode, msg }))
        case 1003:
