...
Layout.layoutBox
    Form.formBox(ref="formInline" :model="formData" :rules="ruleValidate" :label-width="100")
      ...
      FormItem(label="师资介绍：" prop="introduceTeacher")
        VueUeditorWrap(v-model="formData.introduceTeacher" :config="config")
...
export default {
  ...
  methods: {
    ...
    publishLast () {
      ...
      if (courseId === 'add') {
        _addOfflineCourse(this.saveData).then(res => {
          this.$Message.success('发布课程成功')
          // 拿到发布完成课程后的id
          let courseId = res.id
          this.$router.push({ path: `/course/course_timetable`, query: { courseId: courseId, isNewtable: 1 } })
        })
      } else {
        _updateOfflineCourse(this.saveData, courseId).then(res => {
    ...
    saveAddParent () {
      ...
      if (this.parentId) {
        _update(this.parentId, { name: this.parentName }).then(res => {
    ...
    saveAddSub () {
      ...
      if (this.childId) {
        _update(this.childId, { name: this.subName, pid: this.pid }).then(res => {
