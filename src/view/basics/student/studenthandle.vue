<template lang="pug">
  Card.teacherhandle
    ...
    Form(ref="form" :model="form" :rules="rules" :label-width="120" style="width: 960")
      Card(title="添加/编辑学生信息" dis-hover :bordered="false")
        Row
          Col(span="12")
            FormItem(label="学生姓名" prop="name" style="width: 450px")
              Input(v-model="form.name")
        Row
          Col(span="12")
            FormItem(label="性别" prop="tel")
              Input(v-model="form.tel" style="width: 330px")
          Col(span="4")
            //- Button(type="primary" @click="handleRepeatCheck") 查重
        Row
          Col(span="12")
            FormItem(label="教育经历:" style="font-weight: bold;")
        Row
          Col(span="12")
            FormItem(label="幼儿园入学")
              Input(v-model="form.one" style="width: 330px")
          Col(span="12")      
            FormItem(label="小学入学")
              Input(v-model="form.one" style="width: 330px")
        Row
        ...
        Card(title=" " dis-hover :bordered="false")


        Row
          Col(span="12")
            FormItem(label="学生情况" prop="ndtype" style="width: 750px")
              VueUeditorWrap(v-model="form.profile" :config="config")
        Row
          Col(span="12")
            FormItem(label="特殊情况" prop="ndtypenum" style="width: 750px")
              VueUeditorWrap(v-model="form.profile" :config="config")
      Card(title="监护人信息" dis-hover :bordered="false")
        Row
          Col(span="12")
            FormItem(label="监护人" prop="cardholder" style="width: 450px")
              Select(v-model="form.cdtype" style="width: 330px")
                Option(v-for="(v,k) of searchData" :key="k" :value="v.value") {{ v.label }}
            
        Row
          Col(span="12")
            FormItem(label="监护人名称" prop="cardholder" style="width: 450px")
        ...
        Row
          Col(span="12")
            FormItem(label="邮件" prop="moneycard" style="width: 450px")
              Input(v-model="form.moneycard")
        Row
          Col(span="12")
            FormItem(label="工作单位 " prop="bank" style="width: 450px")
              Input(v-model="form.bank")
        Row
          Col(span="12")
            FormItem(label="岗位职务" prop="cdtype")
              Input(v-model="form.cardholder")
        Row
          Col(span="12")
            FormItem
