...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ...
        columns: [
          ...
          { title: '教师姓名（重名+手机号后四位）', key: 'name', width: 120, fixed: 'left'},
          { title: '手机号', key: 'tel', width: 120 },
          { title: '身份证', key: 'ndtypenum', width: 120 },
          { title: '类型', render: (h, { row: { job } }) => h('span', this.getJob(job)) , width: 70 },
          { title: '所在学校（兼职）', key: 'incampus', width: 120 },
          { title: '科目', render: (h, { row: { subject } }) => h('span', this.getSubject(subject)), width: 70},
          { title: '所在校区', render: (h, { row: { school } }) => h('span', this.getCampus(school)), width: 200},
          { title: '添加时间', key: 'senddateFm', width: 120, width: 120 },
          { title: '操作',
            key: 'handle',
            width: 240,
            buttons: [
              { name: 'edit', text: '详情'},
              { name: 'detail', text: '薪酬管理' },
              { name: 'detail', text: '教师课表' },
              {
                name: 'custom',
                render: (h, { row }, vm) => {
                  return h('Button', { props: { type: row.jobswitch === 0 ? 'warning' : 'info', size: 'small' }, on: { click: () => vm.$emit('on-online', row) } }, row.jobswitch === 0 ? '禁用' : '启用')
                }
              }
            ],
            width: 200
    ...
    methods: {
      ...
      handleAdd () {
        console.log("跳转")
        this.$router.push('/basics/teacher/teacherhandle/add')
      },
      handleEdit ({ row }) {
        console.log("编辑")
        this.$router.push(`/basics/teacher/teacherhandle/` + row.id)
      },
